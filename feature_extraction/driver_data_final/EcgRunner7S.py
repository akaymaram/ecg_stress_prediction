...
def main(args):
	...
	for file in args.file:
		...
		if args.auto:
			ecg.auto_invert()
		if args.invert: ecg.invert()
		ecg.scale_outliers()
		ecg.normalize()
		
		min_index_to_plot = args.index_to_plot
		all_peaks = []
		all_peaks = ecg.find_all_peaks(print_verbosity = 0, min_index_to_plot = min_index_to_plot, how_many_previous_peaks_to_use = 10, input_verbosity = 1)
		indices = pd.Series(all_peaks)
		y_values = pd.Series(ecg.total_peak_raw_ecg_values)
		new_list = []
		x = 0
		while x < len(indices):
			new_list.append((indices[x], round(y_values[x], 5)))
			x+=1

		df = pd.concat([indices, y_values], axis = 1)
		df = pd.DataFrame({'peak_index': indices, 'peak_raw_ecg_value': y_values})
		if not os.path.isdir('peak_indices'):
			os.mkdir('peak_indices')
		file_name = 'peak_indices/' + ecg.driver_file_name[:-4] + '_peak_indices.csv'
		if os.path.isfile(file_name):
			old_df = pd.read_csv(file_name)
			x = 0
			old_list = []
			while x < len(old_df.iloc[:,0]):
				old_list.append((old_df.iloc[x,0], round(old_df.iloc[x,1], 5)))
				x+=1
			...
			if len(old_minus_new) == 0 and len(new_minus_old) == 0:
				...
				x = ecg_input(['', ' '], 'go through differences peak by peak? (ENTER for yes, SPACE ENTER for no)')
				if x == '':
					for x in old_minus_new_list:
						closest_index_in_new = find_nearest(indices, x[0])
						if abs(closest_index_in_new-x[0]) < ecg.sampling_rate:
							plt.plot(closest_index_in_new, ecg.ecg_data[closest_index_in_new], 'x', label = 'new peak', color = 'r')
						ecg.plot_index([x[0]], ['old peak'])
						x = ecg_input(['',' '], message = 'press ENTER for next. SPACE ENTER to exit the comparison mode.')
						if x == '':
							plt.close('all')
						elif x == ' ':
							plt.close('all')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rate',  type=int, default=15.5, help='Sampling rate of input file (default: 16)')
    ...
    parser.add_argument('--timeCol',  type=int, default=0, help='Column in CSV file that has Time (default: 0)')
    parser.add_argument('--ecgCol',  type=int, default=1, help='Column in CSV file that has ECG (default: 1)')
